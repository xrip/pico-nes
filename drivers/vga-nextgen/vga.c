...
void __not_in_flash_func(dma_handler_VGA)() {
    ...
    if (line_active >= N_lines_visible) {
        ...
        if ((line_active == N_lines_visible) | (line_active == N_lines_visible + 3)) {
            register uint32_t* ptr_vbuf_OUT = lines_pattern[2 + (line_active & 1)];  
            ptr_vbuf_OUT += shift_picture2 >> 1; 
            register int p_i = ((line_active & is_flash_line) + (frame_i & is_flash_frame)) & 1;
            register uint32_t color32 = bg_color[p_i]; 
            for(register int i = visible_line_size >> 1; i--; ) {
    ...
    if (!vbuf) { // если нет видеобуфера - рисуем пустую строку
        dma_channel_set_read_addr(dma_chan_ctrl, &lines_pattern[0], false);
        return;
    }
    int line  = 0;
    ...
    switch (mode_VGA) {
      ...
      case VGA640x480_text_80_30:
        ...
        int j_ch = i_ch * chrs_in_line;     // номер символа в буфере
        uint8_t* line_tex_buf = &text_buf[j_ch]; // указатель откуда начать считывать символы
        uint8_t* line_tex_buf_color = &text_buf_color[j_ch]; // указатель откуда начать считывать цвета символов
        uint8_t col[2];
        for(int i = 0; fnt8x16 && i < chrs_in_line; ++i)
        {
            register uint8_t d = fnt8x16[(*line_tex_buf++)*font_H + sh_ch]; // из таблицы символов получаем "срез" текущего символа
    ...
    vbuf_OUT += shift_picture2; // смещение началы вывода на размер синхросигнала
    // для div_factor 2
    ...
    int N_loop = visible_line_size - (g_buf_shx > 0 ? g_buf_shx : 0);
    N_loop = MIN(N_loop, max_loop);
    if (N_loop < 0) {
        return;
    }
...
void setVGAmode(enum VGA_mode_t modeVGA) {
    if (_SM_VGA < 0) {
        return; // если VGA не инициализирована
    }
    ...
    switch (modeVGA)
    {
      case VGA640x480_text_80_30:
        ...
        text_buf_height = 30;
        // текстовая палитра
        const uint16_t pm8 = palette16_mask >> 8;
        for(register int i = 0; i < 16; ++i) {
            txt_palette[i] = (txt_palette[i] & 0x3f) | pm8;
        }
        if (!txt_palette_fast) {
            txt_palette_fast = (uint16_t *)malloc(1024 * sizeof(uint16_t));
            for (register int r4 = 0; r4 < 1024; ) {
                register uint8_t c1 = txt_palette[(r4 >> 2) & 0xf];
                register uint8_t c0 = txt_palette[r4 >> 6];
                txt_palette_fast[r4++] = c0 | (c0 << 8); // TODO: why?
                txt_palette_fast[r4++] = c1 | (c0 << 8);
                txt_palette_fast[r4++] = c0 | (c1 << 8);
                txt_palette_fast[r4++] = c1 | (c1 << 8);
            }
        }
        if (!fnt8x16) {
            size_t sz = sizeof(_fnt8x16) * sizeof(uint8_t);
            fnt8x16 = malloc(sz);
    ...
    if (!lines_pattern_data) { // выделение памяти, если не выделено
        uint32_t div32 = (uint32_t) (fdiv * (1 << 16) + 0.0); // TODO: const
        PIO_VGA->sm[_SM_VGA].clkdiv = div32 & 0xfffff000; // делитель для конкретной sm
        size_t sz = line_size >> 2;
        dma_channel_set_trans_count(dma_chan, sz, false); 
        lines_pattern_data = (uint32_t *)malloc(line_size * sizeof(uint32_t)); // TODO: free
        for(register int i = 0; i < 4; ++i) {
            lines_pattern[i] = &lines_pattern_data[i * sz];
        }
...
void clrScr(uint8_t color) {
    size_t sz = text_buf_height * text_buf_width;
    memset(text_buf, 0, sz);
    memset(text_buf_color, color << 4, sz);
    y = 0;
}
...
void initVGA()
{
    ...
    for (int i = 0; i < 16; ++i)
    {
        register uint8_t i3 = (i >> 3) ? 3 : 2;
        register uint8_t b = (i & 1) ? i3 : 0;
        register uint8_t r = (i & 4) ? i3 : 0;
        register uint8_t g = (i & 2) ? i3 : 0;
        register uint8_t c = (r << 4) | (g << 2) | b;
    ...
    uint sm = _SM_VGA;
    const uint VGA_PINS_OUT = 8;
    for (int i = 0; i < VGA_PINS_OUT ; ++i) {
    ...
    pio_sm_set_consecutive_pindirs(PIO_VGA, sm, beginVGA_PIN, VGA_PINS_OUT, true); // конфигурация пинов на выход
    ...
    sm_config_set_out_pins(&c, beginVGA_PIN, VGA_PINS_OUT);
