...
#------------------------------------
add_library(tinyusb_common_base2 INTERFACE)

target_sources(tinyusb_common_base2 INTERFACE
		${TOP}/src/tusb.c
		${TOP}/src/common/tusb_fifo.c
		)

target_include_directories(tinyusb_common_base2 INTERFACE
		${TOP}/src
		)
...
target_compile_definitions(tinyusb_common_base2 INTERFACE
		CFG_TUSB_MCU=OPT_MCU_RP2040
		CFG_TUSB_OS=${TINYUSB_OPT_OS}
		CFG_TUSB_DEBUG=${TINYUSB_DEBUG_LEVEL}
)

target_link_libraries(tinyusb_common_base2 INTERFACE
	hardware_structs
	hardware_irq
	hardware_resets
	pico_sync
...
#------------------------------------
add_library(tinyusb_device_base2 INTERFACE)
target_sources(tinyusb_device_base2 INTERFACE
...
#------------------------------------
add_library(tinyusb_host_base2 INTERFACE)
target_sources(tinyusb_host_base2 INTERFACE
...
target_compile_definitions(tinyusb_host_base2 INTERFACE
...
# BSP & Additions
#------------------------------------
add_library(tinyusb_bsp2 INTERFACE)
target_sources(tinyusb_bsp2 INTERFACE
	${TOP}/hw/bsp/rp2040/family.c
	)
target_include_directories(tinyusb_bsp2 INTERFACE
	${TOP}/hw
	)
target_link_libraries(tinyusb_bsp2	INTERFACE pico_unique_id)

# tinyusb_additions will hold our extra settings for examples
add_library(tinyusb_additions2 INTERFACE)

target_compile_definitions(tinyusb_additions2 INTERFACE
...
if(LOGGER STREQUAL "RTT" OR LOGGER STREQUAL "rtt")
	...
	target_sources(tinyusb_additions2 INTERFACE
	...
	target_include_directories(tinyusb_additions2 INTERFACE
...
function(family_configure_target TARGET RTOS)
	...
	target_link_libraries(${TARGET} PUBLIC pico_stdlib pico_bootsel_via_double_reset tinyusb_board${RTOS_SUFFIX} tinyusb_additions2)
...
function(family_add_pico_pio_usb TARGET)
	target_link_libraries(${TARGET} PUBLIC tinyusb_pico_pio_usb2)
endfunction()
...
function(family_configure_host_example TARGET RTOS)
	...
	if (TARGET tinyusb_pico_pio_usb2)
...
function(family_configure_dual_usb_example TARGET RTOS)
	...
	target_link_libraries(${TARGET} PUBLIC pico_stdlib tinyusb_device tinyusb_host tinyusb_pico_pio_usb2 )
	rp2040_family_configure_example_warnings(${TARGET})
endfunction()
...
function(check_and_add_pico_pio_usb_support)
	...
	if (COMMAND pico_generate_pio_header AND NOT TARGET tinyusb_pico_pio_usb2)
		#------------------------------------
		...
		if (EXISTS ${PICO_PIO_USB_PATH}/src/pio_usb.c)
			add_library(tinyusb_pico_pio_usb2 INTERFACE)
			target_sources(tinyusb_device_base2 INTERFACE
			...
			target_sources(tinyusb_pico_pio_usb2 INTERFACE
			...
			target_include_directories(tinyusb_pico_pio_usb2 INTERFACE
					${PICO_PIO_USB_PATH}/src
					)

			target_link_libraries(tinyusb_pico_pio_usb2 INTERFACE
					hardware_dma
					hardware_pio
					pico_multicore
					)

			target_compile_definitions(tinyusb_pico_pio_usb2 INTERFACE
					PIO_USB_USE_TINYUSB
					)

			pico_generate_pio_header(tinyusb_pico_pio_usb2 ${PICO_PIO_USB_PATH}/src/usb_tx.pio)
			pico_generate_pio_header(tinyusb_pico_pio_usb2 ${PICO_PIO_USB_PATH}/src/usb_rx.pio)
		endif()
	endif()
endfunction()
